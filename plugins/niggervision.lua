...
if (CLIENT) then
        ...
        for _, ent in pairs(ents.GetAll()) do
            if (IsValid(ent)) then
                if (ent.PACPart) then
                    ...
                    if IsValid(par) and !par:IsEffectActive(EF_NODRAW) then -- since there is no proper way to check if the NPC is dead, we just check if the NPC has a nodraw effect on him
                        if (par:IsPlayer() and !par:Alive()) then continue end
                ...
                elseif ent.Type == "nextbot" or ent:IsNPC() or ent:IsPlayer() then
    ...
    local function NV_ResetEverything()        
        ...
        LocalPlayer():ChatPrint([[You're a big faggot and nigger]])
    end
