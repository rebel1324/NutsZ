...
if (SERVER) then
	function ENT:SpawnFunction(client, trace, class)
		...
		entity:SetPos(trace.HitPos + trace.HitNormal * 64)
		entity:Spawn()
		entity:Activate()
	...
	MODEL.position = Vector(5, 0, 55)
	MODEL.scale = Vector(1, 1, 1)
	ENT.modelData["light1"] = MODEL
	
	local MODEL = {}
	MODEL.model = "models/props_c17/light_decklight01_off.mdl"
	MODEL.angle = Angle(0, 0, 180)
	MODEL.position = Vector(-5, 0, 55)
	MODEL.scale = Vector(1, 1, 1)
	...
	function ENT:Draw()
		for uid, dat in pairs(self.modelData) do
			...
			if (drawEntity and drawEntity:IsValid()) then
				...
				local size = self:GetDTBool(0, false) and math.abs(math.sin(RealTime()*10)) * 64 or 64
