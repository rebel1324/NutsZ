		...
		if (char) then
			...
			if (vgui.CursorVisible() and (terminal == self.entity)) then
				...
				if (self.entity:getNetVar("locked") and !self.access) then
					local tx, ty = draw.SimpleText("P", "nutIconsBig", w/2, h/2, color_white, 1, 4)
					tx, ty = draw.SimpleText("SYSTEM LOCKED", "nutBigFont", w/2, h/2, color_white, 1, 5)
					...
					nut.util.drawText("LOGIN", bp + sp/2, bp2 + sp2/2 - 3, color_white, 1, 1, "nutMediumFont")
				else
					self.access = true
					self.curSel = -1

					local tx, ty = draw.SimpleText(self.entity.title or "WELCOME, USER.", "nutBigFont", w/2, 1 * scale, color_white, 1, 5)
				...
				local tx, ty = draw.SimpleText("F", "nutIconsBig", w/2, h/2, color_white, 1, 4)
				tx, ty = draw.SimpleText("SYSTEM STANDBY", "nutBigFont", w/2, h/2, color_white, 1, 5)
			end
		end
	end
